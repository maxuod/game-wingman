...
- 桌面主窗口、独立置顶浮窗、收起、透明度、点击穿透与恢复入口。浮窗点击和关闭事件不再将其隐藏；置顶在未读取画面时也会持续维持，需要隐藏时用主窗口或托盘菜单。
- 浮窗默认显示、保持置顶且不抢游戏焦点；自动查找唯一游戏窗口并开始本机预览，支持独立 TFT 客户端。
- 主窗口先列出当前补丁、近期有效统计中前四率最高的 4 套阵容。选定后，“本局怎么打”会根据最近识别的阶段、金币、生命和所选阵容给出下一步提示，装备与海克斯另列；浮窗也跟随所选阵容。自动找到唯一 TFT 游戏窗口后开始读取本机画面；已配置并启用 DeepSeek 时会请求本局持续跟进确认。没有读清的字段和装备不猜测，海克斯候选需核对本局实际选项。
- 选择一个窗口后播放实时本机视频；暂停、继续、关闭源窗口后停止；支持导入截图。
- 已有的 Riot Data Dragon 名称字典缓存仅用于后台名称匹配；主界面不再提供独立资料浏览页。
...
主窗口的顺序是选阵容 → 看本局下一步 → 核对装备和海克斯。画面预览默认收起，可展开检查；全部阵容、配装站位和 AI 配置放在次级面板。浮窗跟随所选阵容显示一条建议。

`design/` 中的网页与旧界面截图只保留作早期设计记录；当前运行界面以新便携包为准。
...
普通使用无需编辑源码：打开“设置 → AI 识别 → API Key 配置”，选择平台、粘贴自己的 Key 并加密保存。也可导入本地 `.env` / JSON，或先保存空白模板填写。[GitHub JSON 空模板](api-keys.example.json)只含空字段，实际密钥不会随源码或软件包分发。[完整操作说明](docs/api-providers.md)。
...
以上 CLI 命令只在本地检查配置，默认 `AI_ENABLED=false`。桌面可在“设置 → AI 识别”导入本机密钥文件，以系统加密方式保存；手动识别每张确认，持续跟进另需会话确认。仅开启本机预览不调用模型。不要提交密钥到仓库或 Issue。
...
最新 Windows 包位于 `release/advice-first-preview/Game Wingman-win32-x64/`，先从托盘退出旧版再运行；同名旧实例会拦截新版启动。保留累计 DeepSeek ¥10 预算。后续验证真实 TFT 装备识别与当前客户端的阵容码导入。
