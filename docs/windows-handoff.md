...
最新简洁助手包：`release/advice-first-preview/Game Wingman-win32-x64/Game Wingman.exe`。主界面先选阵容，再看按阶段、金币和生命更新的下一步、装备及海克斯提示；视频预览默认收起。独立“美服资料”浏览页已移除，已有名称字典缓存仍可辅助识别。浮窗仅跟随明确选定的阵容。先从托盘退出旧实例，再启动新包；真实 TFT 对局中的识别和覆盖仍待实测。

最新置顶修复包：`release/pinned-overlay-preview/Game Wingman-win32-x64/Game Wingman.exe`。浮窗点击或关闭事件不会将其隐藏，未读取画面时也维持置顶；需要隐藏可用主窗口或托盘菜单。替换旧运行实例后才能看到修复。窗口化全屏游戏内层级仍需实测。

先前自动跟进版：`release/auto-follow-preview/Game Wingman-win32-x64/Game Wingman.exe`。旧实例必须从托盘退出，单实例锁会使直接双击新 EXE 仍回到旧版。新版主界面显示前四率前 4 套阵容；唯一 TFT 窗口出现后自动读取本机画面。已保存 DeepSeek 密钥且“允许 AI 请求”开启时，自动请求本局持续识别确认；同意后装备与所选阵容的海克斯候选随观察更新。仍使用跨重启累计 ¥10 上限。真实 TFT 识别与全屏覆盖待实测。
...
普通用户打开“设置 → AI 识别 → API Key 配置”，选择平台后粘贴自己的 Key，点击“加密保存密钥”。也可导入本地 `.env` / JSON，或点击“保存空白模板”后填写。仓库提供 `api-keys.example.json` 与 `.env.example`；空字段跳过，导入按平台字段名归属，不猜测 Key 前缀。
