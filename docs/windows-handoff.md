...
最新自动跟进版：`release/auto-follow-preview/Game Wingman-win32-x64/Game Wingman.exe`。旧实例必须从托盘退出，单实例锁会使直接双击新 EXE 仍回到旧版。新版主界面显示前四率前 4 套阵容；唯一 TFT 窗口出现后自动读取本机画面。已保存 DeepSeek 密钥且“允许 AI 请求”开启时，自动请求本局持续识别确认；同意后装备与所选阵容的海克斯候选随观察更新。仍使用跨重启累计 ¥10 上限。真实 TFT 识别与全屏覆盖待实测。
