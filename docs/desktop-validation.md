...
- 发布范围 51 项离线测试通过。隐藏 Electron 合成窗口和 Windows 便携包内回归通过：选择阵容、随阶段更新下一步、装备与海克斯提示、折叠预览仍自动读取、主窗口手动操作及浮窗跟随所选阵容。完整桌面烟雾测试通过。
- 本机另外保留的独立 STS1 研究使完整工作区测试为 57 项，均通过；研究文件未随 TFT 公开版发布。Windows 新 `advice-first-preview` 便携 EXE 已替换旧版运行。检查时无 TFT 对局进程，真实游戏内识别、建议适配及覆盖待实测。所有桌面验证使用合成窗口、假密钥和模拟网络，真实付费 AI 请求为零。

## 浮窗持续置顶（2026-09-24）

- 50 项 TFT 离线测试、隐藏 Electron 合成窗口回归通过。验证浮窗内容点击后仍请求显示、关闭事件不销毁或隐藏、未读取画面时被撤销的置顶标记会自动恢复；主窗口显式隐藏/显示仍正常。
- 原生测试保持窗口隐藏，不夺取用户焦点，也未启动真实游戏或调用付费 API。打包版与真实 TFT 窗口化全屏的实际层级需分别验证。
