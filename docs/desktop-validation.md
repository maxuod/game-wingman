...
## 自动 TFT 跟进与主窗口四套阵容（2026-09-24）

- 50 项 TFT 离线测试和隐藏 Electron 合成窗口回归通过。主窗口显示同补丁、近期有效样本中前四率最高的 4 套，点击可固定目标；榜单失效时不给出假当前排名。
- 模拟唯一 TFT 窗口出现后无需点“开始读取”即可进入本机实时画面。已启用且有密钥时只弹一次本局发送确认；取消后零付费请求，确认后自动取得首个模拟 DeepSeek 观察。装备优先合成随新鲜清单更新，所选阵容的阶段海克斯候选按公开来源展示。
- 所有网络和画面均为合成夹具，真实付费请求为零。报告和截图在 `artifacts/auto-follow-native/`。真实 TFT 的窗口覆盖、识别准确率和本局三选一海克斯匹配仍待验证。
