...
(async () => {
  ...
  const args = [...(process.env.GWM_EXECUTABLE ? [] : [path.resolve('.')]), `--user-data-dir=${profile}`];
  ...
  try {
    ...
    await main.screenshot({ path: 'artifacts/app-main.png' });
    ...
    assert.equal(native.top, true); assert.equal(native.visible, true); assert.deepEqual(native.size, [320, 248]);
    await overlay.screenshot({ path: 'artifacts/app-overlay.png' });
    ...
    if (permission === 'granted') {
      ...
      await main.screenshot({ path: 'artifacts/app-capture.png' });
      await main.click('#capture-button');
    ...
    await main.screenshot({ path: 'artifacts/app-settings.png' });
    ...
    await main.screenshot({ path: 'artifacts/app-overlay-settings.png' });
    await main.click('[data-close="settings-dialog"]');
    await main.evaluate(() => window.desktop.selectEntry(null));
    assert.deepEqual(failures, []);
    await fs.writeFile('artifacts/smoke-report.json', JSON.stringify(report, null, 2));
