const { PROVIDERS, providerConfig, generateText } = require('../dist/main/ai/providers.js');
async function main() {
  ...
  console.log(JSON.stringify({ provider: config.provider, model: config.model, enabled: config.enabled,
    keyConfigured: Boolean(config.apiKey), endpoint: PROVIDERS[config.provider].url, mode: args.includes('--probe') ? 'live text probe' : 'configuration only; no network request' }, null, 2));
